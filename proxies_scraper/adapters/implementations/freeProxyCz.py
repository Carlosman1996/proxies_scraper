...
class FreeProxyList(Proxies):
        ...
        for table_rows_element in table_rows_elements:
            ...
            if len(row_cells_elements) == 11:
                ...
                for index_cell, row_cells_element in enumerate(row_cells_elements):
                    if index_cell == 0:
                        proxy_model[self._proxy_table_indexes[index_cell]] = (
                            proxies_ips[iteration]
                        )
                    else:
                        proxy_model[self._proxy_table_indexes[index_cell]] = (
                            row_cells_element.text
                        )
                proxy_model["proxy"] = (
                    proxy_model["IP_Address"] + ":" + proxy_model["Port"]
                )
                proxy_model["created_date"] = datetime.now(TIMEZONE_MADRID)
                proxy_model["Anonymity"] = Common.type_converter(
                    proxy_model["Anonymity"], parameter_type="Anonymity"
                )
                proxy_model["Https"] = Common.type_converter(
                    proxy_model["Protocol"], parameter_type="Protocol"
                )
    ...
    def get_proxies(self) -> list:
        ...
        for url in self.urls:
            ...
            # Get pages:
            paginator_elements = driver.find_element(
